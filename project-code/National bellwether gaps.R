...
wd <- "C:/Users/danw/Box/Gap Narrative (ITEP), June 2019/git/gap-narrative/"
setwd(wd)
...
for(iso in countries){
  ...
  pop.weights <- merge(pop.weights, GP20[CountryCode==iso], by.x="year",by.y="RequestYear")
  pop.weights <- pop.weights[,c(1:5,9)]
  pop.weights.melt <- melt(pop.weights, id.vars = c("V2","year","ReqYearPopulation"))
  ...
  dhs.melt <- melt(dhs[iso3==iso], id.vars=c("variable","p20","iso3","type","povcal_year","survey_year","sex"))
  ...
  dhs.cast <- merge(dhs.cast, pop.weights.melt, by.x=c("variable","povcal_year","sex"), by.y=c("V2","year","sex"))
  
  dhs.dt <- dhs.cast[, .(P20.value = P20.share*ReqYearPopulation*weight, U80.value = U80.share*ReqYearPopulation*weight, P20.pop = ReqYearPopulation*weight*(TRUE_denominator/(FALSE_denominator+TRUE_denominator)), U80.pop=ReqYearPopulation*weight*(FALSE_denominator/(FALSE_denominator+TRUE_denominator))), by=.(povcal_year, variable, sex)]
  ...
  fwrite(dhs.dt, paste0("output/",iso," DHS gaps analysis.csv"), row.names = F)
  ...
  for(var in colnames(dhs.dt[,4:7])){
    ...
    ggsave(paste0("output/",iso," ",var," combined.png"), width = 8.27, height = 5.83)
  }
}