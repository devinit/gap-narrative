...
countries <- c("NPL","UGA","KEN","BEN")

for(iso in countries){
  u5.pop <- as.data.table(WDI(country=iso, indicator = c("SP.POP.0004.MA.5Y","SP.POP.0004.FE.5Y","SP.POP.TOTL.MA.ZS","SP.POP.TOTL.FE.ZS"), start=1990, end=2015, extra = T))
  ...
  f.1549.fert <- as.data.table(WDI(country=iso, indicator = c("SP.POP.1519.FE.5Y","SP.POP.2024.FE.5Y","SP.POP.2529.FE.5Y","SP.POP.3034.FE.5Y","SP.POP.3539.FE.5Y","SP.POP.4044.FE.5Y","SP.POP.4549.FE.5Y","SP.POP.TOTL.FE.ZS","SP.DYN.TFRT.IN"), start=1990, end=2015, extra=T))
  ...
  iso.melt <- melt(subset(dhs, iso3 == iso), id.vars=c("variable","p20","iso3","type","povcal_year","survey_year"))
  iso.cast <- dcast.data.table(iso.melt, variable + iso3 + povcal_year ~ p20 + type)
  iso.cast$P20.share <- iso.cast$TRUE_numerator/(iso.cast$TRUE_denominator+iso.cast$FALSE_denominator)
  iso.cast$U80.share <- iso.cast$FALSE_numerator/(iso.cast$TRUE_denominator+iso.cast$FALSE_denominator)
  
  iso.cast <- merge(iso.cast, pop.weights, by.x=c("variable","iso3","povcal_year"), by.y=c("V2","iso3c","year"))
  
  iso.dt <- iso.cast[, .(P20.value = P20.share*weight, U80.value = U80.share*weight, P20.pop = weight*(TRUE_denominator/(FALSE_denominator+TRUE_denominator)), U80.pop=weight*(FALSE_denominator/(FALSE_denominator+TRUE_denominator))), by=.(country, povcal_year, variable)] 
  iso.dt <- iso.dt[,.(P20.value=sum(P20.value, na.rm=T)/sum(P20.pop, na.rm=T),U80.value=sum(U80.value, na.rm=T)/sum(U80.pop, na.rm=T)),by=.(povcal_year,variable)]
  
  #Cast to pretty data.table
  iso.dt <- melt(iso.dt, id.vars=c("povcal_year","variable"))
  iso.dt$gp20 <- "P20"
  iso.dt[which(iso.dt$variable.1 == "U80.value")]$gp20 <- "Rest of population"
  iso.dt <- dcast.data.table(iso.dt, povcal_year + gp20 ~ variable)
  
  colnames(iso.dt) <- c("year","gp20","Secondary education","Under-5 mortality","Birth registration","Stunting")
  
  write.csv(iso.dt, paste0("output/gaps/ ",iso," gaps analysis.csv"), row.names = F)
  ...
  for(var in colnames(iso.dt[,3:6])){
    varen <- as.name(var)
    varen <- enquo(varen)
    p <- ggplot(subset(iso.dt, year >=1990),aes(year))+
      ...
      scale_y_continuous(
        limits = c(0,round(max(iso.dt[,..var])*1.2,2)),
        expand = c(0,0),
    ...
    ggsave(paste0("output/gaps/",iso," ",var,".png"), width = 8.27, height = 5.83)
  }
}
